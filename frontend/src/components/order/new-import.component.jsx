...
export default class NewOrder extends Component {
  ...
  handlePlaceOrder(e) {
    ...
    if (this.checkBtn.context._errors.length === 0) {
        let importRequest = {userId: this.state.user.id,
                            note: this.state.note,
                            productIds: this.state.productIds,
                            quantities: this.state.quantities,
                            isPaid: this.state.isPaid}

        var result = window.confirm("Want to place this import?");
        if (result) {
          importService.placeImport(importRequest).then(
            response => {
              this.setState({
            ...
            {!this.state.successful && (
              <div>
                ...
                <div className="row justify-content-center form-group">
                  <button className="btn btn-primary btn-block col-sm-2 justify-content-center">Place Import</button>
                </div>
              </div>
            )}
